...
import { depositToWallet, getSellerSavedCards } from "../../../API/seller";

export default function Seller_deposit() {
  ...
  useEffect(() => {
    document.title = t("depositDocTitle");
  }, [t]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMessage(res?.message || t("depositSuccessful"));
      setAmount("");
    } catch (err) {
      setError(
  ...
  return (
    <div className="deposit-page" dir={isArabic ? "rtl" : "ltr"}>
      <div className="deposit-container">
        <h1 style={{ fontWeight: "bold" }}>{t("depositTitle")}</h1>
        ...
        <div className="deposit-section">
          <div className="deposit-section-title">{t("quickDeposit")}</div>
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? <span className="btn-spinner"></span> : t("depositTitle")}
        </button>
      </div>
    </div>
  );
