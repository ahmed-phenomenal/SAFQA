...
export default function Seller() {
  ...
  const [showWelcomeVerificationModal, setShowWelcomeVerificationModal] =
    useState(false);
  ...
  useEffect(() => {
    document.title = t("sellerDocTitle");
  }, [t]);
  ...
  const syncVerificationState = useCallback(async () => {
    try {
      ...
      if (!normalized.isVerified && !normalized.isPending) {
  ...
  const openVerificationModal = (auctionType) => {
  ...
  const getAuctionRoute = (auctionType) =>
    auctionType === "lot" ? LOT_AUCTION_ROUTE : SINGLE_AUCTION_ROUTE;

  const getAuctionLabel = (auctionType) =>
    auctionType === "lot" ? t("newLotAuctionLower") : t("newSingleAuctionLower");
      ...
      <div className="seller-page">
        <div className="seller-container">
          <div className="seller-header">
            <h1 className="seller-logo">
              SAFQA<span>.BUSINESS</span>
            </h1>
          </div>
              ...
              {!bannerCollapsed ? (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {isPendingOrSubmitted
                        ? t("verificationPendingBannerText")
                        : t("verificationRequiredBannerText")}
                    </p>
                  </div>
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        navigate(VERIFICATION_ROUTE, {
                          state: {
                            mode: isPendingOrSubmitted ? "review" : "verify",
                          },
                        })
                      }
                      style={{
                        ...
                        fontSize: "16px",
                        cursor: "pointer",
                        boxShadow: "0 6px 14px rgba(0, 63, 152, 0.18)",
                        whiteSpace: "nowrap",
                      }}
                    >
                    ...
                    <button
                      ...
                      aria-label={t("hideVerificationBanner")}
                      style={{
                        border: "none",
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => setBannerCollapsed((prev) => !prev)}
                    aria-label={t("showVerificationBanner")}
                    style={{
                      border: "none",
                      background: "transparent",
                      color: isPendingOrSubmitted ? "#0b4aa2" : "#b36b00",
          ...
          <div className="seller-content">
            <button
              type="button"
              onClick={() => handleAuctionClick("lot")}
              className="seller-card seller-card-large seller-card-button"
            ...
            >
              ...
              <div className="seller-card-text seller-card-text-large">
                {verificationChecking && selectedAuctionType === "lot" ? (
                  t("checking")
                ) : (
                  <>
                    {t("newLotAuction")} <span>+</span>
                  </>
                )}
              </div>
            </button>
            ...
            <button
              type="button"
              onClick={() => handleAuctionClick("single")}
              className="seller-card seller-card-large seller-card-button"
              disabled={verificationChecking}
            >
              ...
              <div className="seller-card-overlay"></div>
              <div className="seller-card-text seller-card-text-large">
                {verificationChecking && selectedAuctionType === "single" ? (
                  t("checking")
                ) : (
                  <>
                    {t("newSingleAuction")} <span>+</span>
                  </>
                )}
              </div>
            </button>
            ...
            <div className="seller-small-grid">
              ...
              <Link
                ...
                className="seller-card seller-card-small"
              >
                <img src={statisticsImg} alt={t("statistics")} />
                <div className="seller-card-overlay"></div>
                <div className="seller-card-text seller-card-text-small">
                  {t("statistics")}
                </div>
        ...
        !verificationStatus?.hasSubmittedVerification && (
          ...
          >
            ...
            >
              ...
              >
                {t("completeVerificationBusinessText")}
              </p>
        ...
        <div className="seller-confirm-overlay">
          <div className="seller-confirm-modal">
            ...
            <p>
              {isPendingOrSubmitted ? (
                <>
                  {t("verificationUnderReviewBeforeAuction")}{" "}
                  <strong>{getAuctionLabel(selectedAuctionType)}</strong>.
                </>
              ) : (
                <>
                  {t("needVerificationBeforeAuction")}{" "}
                  <strong>{getAuctionLabel(selectedAuctionType)}</strong>.
                </>
              )}
            </p>
            ...
            <div className="seller-confirm-actions">
              <button
                className="seller-confirm-btn seller-confirm-cancel"
              ...
              <button
                className="seller-confirm-btn seller-confirm-ok"
                onClick={handleConfirmVerification}
              >
                {isPendingOrSubmitted ? t("review") : t("verifyNow")}
              </button>
