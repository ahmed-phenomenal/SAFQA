...
const normalizeEndingSoonItem = (item, index) => ({
  id: Number(
    item?.auctionId ?? item?.AuctionId ?? item?.id ?? item?.Id ?? index + 1
  ),
  title: String(item?.title ?? item?.Title ?? "Auction"),
  image: getImageSrc(item?.image ?? item?.Image, FALLBACK_AUCTION_IMAGE),
  currentPrice: Number(
    item?.currentPrice ?? item?.CurrentPrice ?? item?.displayPrice ?? 0
  ),
...
const normalizeCategoryItem = (item, index) => {
  ...
  const fallback =
    FALLBACK_CATEGORY_IMAGES[name] || FALLBACK_CATEGORY_IMAGES.default;
...
const AuctionMiniCard = ({ item, t, onOpen }) => {
    ...
    >
      <div className="home-auction-mini-image-wrap">
        <img
          src={item.image}
          alt={item.title}
          className="home-auction-mini-image"
        />
      ...
      <div className="home-auction-mini-body">
        ...
        <div className="home-auction-mini-price">
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchHomeData = async () => {
      ...
      const requests = [
        api.get("/User/ending-soon", {
          params: { page: 1, pageSize: 10 },
          timeout: 60000,
        }),
        api.get("/User/trending", {
          params: { page: 1, pageSize: 10 },
          timeout: 60000,
        }),
      ...
      const [endingRes, trendingRes, categoriesRes, recommendationsRes] =
      ...
      if (trendingRes.status === "fulfilled") {
        const raw = normalizeListResponse(trendingRes.value?.data);
  ...
  const scrollToRealIndex = (realIndex, behavior = "smooth") => {
    ...
    const normalized =
      ((realIndex % endingSoon.length) + endingSoon.length) % endingSoon.length;

    if (normalized !== current) {
      const forwardDistance =
        (normalized - current + endingSoon.length) % endingSoon.length;
      const backwardDistance =
        (current - normalized + endingSoon.length) % endingSoon.length;
  ...
  const normalizeAfterScroll = () => {
    ...
    if (realIndex < 0) {
      realIndex = endingSoon.length + realIndex;
      scrollToRealIndex(realIndex, "auto");
      return;
    }

    if (realIndex >= endingSoon.length) {
      realIndex = realIndex - endingSoon.length;
      scrollToRealIndex(realIndex, "auto");
      return;
    }
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      scrollToVirtualIndex(cloneSize + endingSoonIndex, "auto");
    });
  ...
  const handleEndingSoonScroll = () => {
    ...
    scrollEndTimerRef.current = setTimeout(() => {
      if (!isDraggingRef.current && !programmaticScrollRef.current) {
  ...
  const handleTouchStart = (e) => {
    ...
    if (!touch) return;
    handlePointerDown(touch.pageX);
  };

  const handleTouchMove = (e) => {
    const touch = e.touches?.[0];
    if (!touch) return;
    handlePointerMove(touch.pageX);
  };
  ...
  const renderFiveDots = () => {
    ...
    return dots.map((offset) => {
      const dotIndex =
        ((endingSoonIndex + offset) % endingSoon.length + endingSoon.length) %
        endingSoon.length;
      ...
      return (
        <button
          ...
          type="button"
          className={`home-ending-dot home-ending-dot-window ${
            offset === 0 ? "active" : ""
          } dot-offset-${offset + 2} dot-moving-${dotDirection}`}
          onClick={() => {
            scrollToRealIndex(endingSoonIndex + offset, "smooth");
            pauseAutoAfterManual();
          }}
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .home-ending-soon-slider::-webkit-scrollbar {
          display: none;
        }
        ...
        .home-ending-dot.active {
          width: 24px;
          background: #0b4aa2;
        }

        .home-ending-dot-window.dot-offset-0,
        .home-ending-dot-window.dot-offset-4 {
          opacity: 0.65;
          transform: scale(0.82);
        }

        .home-ending-dot-window.dot-offset-1,
        .home-ending-dot-window.dot-offset-3 {
          opacity: 0.85;
          transform: scale(0.92);
        }

        .home-ending-dot-window.dot-moving-next {
          animation: dotSlideNext 0.25s ease;
        }

        .home-ending-dot-window.dot-moving-prev {
          animation: dotSlidePrev 0.25s ease;
        }
                      ...
                      {endingSoonSlides.map((item, index) => (
                        ...
                        >
                          <div
                            ...
                            onKeyDown={(e) => {
                              if (e.key === "Enter") openAuctionDetails(item.id);
                            }}
                          >
                            <div className="home-ending-image-box">
                              <img src={item.image} alt={item.title} />
                              <span className="home-ending-timer">
                                {getTimeLeft(item.endDate, t)}
                              </span>
                            </div>
