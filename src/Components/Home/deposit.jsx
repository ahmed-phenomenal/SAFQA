...
export default function Deposit() {
  ...
  useEffect(() => {
    document.title = t("depositDocTitle", "Deposit");
  }, [t]);
  ...
  useEffect(() => {
    ...
    const loadCards = async () => {
      try {
        const cardsData = await getUserSavedCards();
  ...
  const handleAmountChange = (e) => {
    ...
    if (value && Number(value) < 100) {
      setError(t("amountMin100", "Amount must not be less than 100 EGP"));
    } else {
      setError("");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedCardId) {
      setError(t("pleaseChooseCard", "Please choose your card"));
      return;
    }
    ...
    try {
      const res = await depositToUserWallet({
        amount: numericAmount,
        savedCardId: selectedCardId,
      });

      setMessage(res?.message || t("depositSuccessful", "Deposit successful"));
      setAmount("");
    } catch (err) {
      setError(
        err?.response?.data?.message ||
          err?.message ||
          t("depositFailed", "Deposit failed")
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="deposit-page" dir={isArabic ? "rtl" : "ltr"}>
      <div className="deposit-container">
        ...
        <div className="deposit-section">
          <div className="deposit-section-title">
            {t("chooseYourCard", "Choose your card")}
          </div>
          ...
          {cards.length > 0 ? (
            ...
            <div className="deposit-card active">
              <div className="deposit-card-left">
                <div className="deposit-card-info">
                  <div className="deposit-card-name">
                    {t("noSavedCardsFound", "No saved cards found")}
                  </div>
                  <div className="deposit-card-number">
                    {t("pleaseAddCardFirst", "Please add a card first")}
                  </div>
                </div>
              </div>
            </div>
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? <span className="btn-spinner"></span> : t("depositTitle", "Deposit")}
        </button>
      </div>
    </div>
  );
